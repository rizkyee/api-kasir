    ...
    public function updateTransaksi($request, $id)
    {
        ...
        try {
            ...
            foreach ($request->items as $item) {
                $produk = $this->repo->findProduk($item['id_produk']);
                if ($produk->stok < $item['jumlah']) {
                    return ['error' => 'Stok tidak cukup untuk ' . $produk->nama_produk, 'status' => 422];
                }
                $total_harga += ($produk->harga * $item['jumlah']);
            }

            // Update transaksi
            $transaksi = $this->repo->updateTransaksi($id, [
                'id_pelanggan' => $request->id_pelanggan,
                'id_metode'    => $request->id_metode,
                'status'       => $request->status,
                'total_harga'  => $total_harga
            ...
            // Hapus detail lama & kembalikan stok
            foreach ($transaksi->detail as $detail) {
                $produk = $this->repo->findProduk($detail->id_produk);
